...
├── appsettings.json                  # Connection string, API key config
...
Open `appsettings.json` and update these two values:

```json
{
  "ConnectionStrings": {
    "DefaultConnection": "Server=localhost;Database=PipelineHealthMonitor;Trusted_Connection=True;TrustServerCertificate=True;"
  },
  "Auth": {
...
The key is configured in `appsettings.json`. In production, inject it via environment variable or a secrets manager — never commit a real key to source control.
